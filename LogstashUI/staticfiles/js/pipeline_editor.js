...
document.addEventListener('DOMContentLoaded', function () {
    // Add event delegation for the insertion point buttons
    document.addEventListener('click', function(e) {
        // Handle Add Plugin button in insertion points
        if (e.target.closest('.insertion-button.add-plugin')) {
            e.preventDefault();
            e.stopPropagation();
        ...
        else if (e.target.closest('.insertion-button.add-condition')) {
            e.preventDefault();
            e.stopPropagation();
            const button = e.target.closest('.insertion-button.add-condition');
            const type = button.dataset.type;
            addConditionalBlock(type);
        }
    });
