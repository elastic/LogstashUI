...
class TestConnectionCRUD:
    ...
    @patch("PipelineManager.manager_views.test_connectivity")
    def test_add_connection_failed_connectivity(
        self, mock_test_connectivity, authenticated_client
    ):
    ...
    def test_add_connection_invalid_form(self, authenticated_client):
        ...
        assert (
            "name" in response_data["error"]
            or "This field is required" in response_data["error"]
        )
    ...
    def test_delete_connection(self, authenticated_client, test_connection):
        ...
        response = authenticated_client.post(
            f"/ConnectionManager/DeleteConnection/{connection_id}/"
        )
    ...
    def test_delete_nonexistent_connection(self, authenticated_client):
        ...
        response = authenticated_client.post(
            "/ConnectionManager/DeleteConnection/99999/"
        )
...
class TestPipelineCRUD:
    ...
    def test_create_pipeline_success(
        ...
        response = authenticated_client.post(
            "/ConnectionManager/CreatePipeline/",
            {
    ...
    def test_delete_pipeline_success(
        ...
        response = authenticated_client.post(
            ...
            {"es_id": test_connection.id, "pipeline": "test_pipeline"},
        )
    ...
    def test_update_pipeline_settings_success(
        self, mock_get_pipeline, mock_get_es, authenticated_client, test_connection
    ):
        ...
        response = authenticated_client.post(
            "/ConnectionManager/UpdatePipelineSettings/",
            {
                "es_id": test_connection.id,
                "pipeline": "test_pipeline",
...
class TestPipelineNameValidation:
    ...
    @patch("PipelineManager.pipelines_crud.get_elastic_connection")
    def test_pipeline_name_starts_with_letter(
        self, mock_get_es, authenticated_client, test_connection
    ):
        ...
        response = authenticated_client.post(
            "/ConnectionManager/CreatePipeline/",
            {"es_id": test_connection.id, "pipeline": "valid_pipeline"},
        )

        assert response.status_code == 200

    @patch("PipelineManager.pipelines_crud.get_elastic_connection")
    def test_pipeline_name_starts_with_underscore(
        self, mock_get_es, authenticated_client, test_connection
    ):
        ...
        response = authenticated_client.post(
            "/ConnectionManager/CreatePipeline/",
            {"es_id": test_connection.id, "pipeline": "_valid_pipeline"},
        )

        assert response.status_code == 200

    def test_pipeline_name_starts_with_number_invalid(
        self, authenticated_client, test_connection
    ):
        """Test that pipeline name cannot start with a number"""
        response = authenticated_client.post(
            "/ConnectionManager/CreatePipeline/",
            {"es_id": test_connection.id, "pipeline": "123invalid"},
        )

        assert response.status_code == 400
        assert b"must begin with a letter or underscore" in response.content

    def test_pipeline_name_special_chars_invalid(
        self, authenticated_client, test_connection
    ):
        """Test that pipeline name cannot contain special characters"""
        response = authenticated_client.post(
            "/ConnectionManager/CreatePipeline/",
            {"es_id": test_connection.id, "pipeline": "invalid@pipeline"},
        )
        ...
        assert (
            b"can only contain letters" in response.content
            or b"underscores, dashes, hyphens" in response.content
        )
    ...
    def test_pipeline_name_empty_invalid(self, authenticated_client, test_connection):
        ...
        response = authenticated_client.post(
            "/ConnectionManager/CreatePipeline/",
            {"es_id": test_connection.id, "pipeline": ""},
        )
...
class TestSecurityAndErrorHandling:
    ...
    @patch("PipelineManager.manager_views.test_connectivity")
    def test_connectivity_error_message_escaped(
        self, mock_test_connectivity, authenticated_client, test_connection
    ):
    ...
    @patch("PipelineManager.manager_views.test_connectivity")
    def test_readonly_user_cannot_add_connection(
        self, mock_test_connectivity, client, test_user
    ):
        ...
        readonly_user = User.objects.create_user(
            username="readonly_add", password="testpass123", is_staff=False
        )
    ...
    def test_readonly_user_cannot_delete_connection(self, client, test_connection):
        ...
        readonly_user = User.objects.create_user(
            username="readonly_delete", password="testpass123", is_staff=False
        )
        # Update the auto-created profile to readonly role
        # (post_save signal creates profile with 'admin' role by default)
        readonly_user.profile.role = "readonly"
...
class TestConnectivityHelper:
    ...
    @patch(
        "PipelineManager.manager_views.get_elastic_connection",
...
class TestTestConnectivityView:
    ...
    def test_success_renders_green_div(
        self, mock_tc, authenticated_client, test_connection
    ):
...
class TestConnectionMethodGuards:
    ...
    def test_delete_connection_get_returns_405(
        self, authenticated_client, test_connection
    ):
...
class TestGetPipelineEndpoint:
    ...
    def test_pipeline_not_found_returns_400(
        self, mock_glp, authenticated_client, test_connection
    ):
        response = authenticated_client.get(
...
class TestClonePipelineEdgeCases:
    ...
    def test_invalid_source_name_returns_400(
        ...
        response = authenticated_client.post(
            "/ConnectionManager/ClonePipeline/",
            {
                "es_id": test_connection.id,
    ...
    @patch("PipelineManager.pipelines_crud.get_elastic_connection")
    def test_source_pipeline_not_found_returns_404(
        self, mock_get_es, authenticated_client, test_connection
    ):
        mock_es = MagicMock()
    ...
    @patch("PipelineManager.pipelines_crud.get_elastic_connection")
    def test_new_pipeline_name_already_exists_returns_400(
        self, mock_get_es, authenticated_client, test_connection
    ):
        mock_es = MagicMock()
    ...
    @patch(
        "PipelineManager.pipelines_crud.get_elastic_connection",
        side_effect=Exception("ES down"),
    )
    def test_clone_exception_returns_500(
        self, mock_get_es, authenticated_client, test_connection
    ):
        response = authenticated_client.post(
            "/ConnectionManager/ClonePipeline/",
...
class TestDeletePipelineEdgeCases:
    ...
    def test_invalid_pipeline_name_returns_400(
        self, authenticated_client, test_connection
    ):
        response = authenticated_client.post(
            "/ConnectionManager/DeletePipeline/",
...
class TestIntegration:
    ...
    def test_full_pipeline_lifecycle(
        self, mock_get_es, mock_test_connectivity, authenticated_client
    ):
        ...
        create_response = authenticated_client.post(
            "/ConnectionManager/CreatePipeline/",
            {"es_id": connection.id, "pipeline": "integration_test_pipeline"},
        )
        ...
        delete_conn_response = authenticated_client.post(
            f"/ConnectionManager/DeleteConnection/{connection.id}/"
        )
...
class TestCreatePipelineAdditional:
    ...
    @patch(
        "PipelineManager.pipelines_crud.requests.put",
        side_effect=__import__("requests").exceptions.ConnectionError("agent down"),
    )
    def test_simulate_mode_failure_returns_500(
        self, mock_put, authenticated_client, settings
    ):
...
class TestAgentStatusStream:
    ...
    def test_payload_carries_the_running_logstash_version(
    ...
    def test_payload_version_is_null_when_never_reported(
        self, authenticated_client, db
    ):
