...
(function () {
  ...
  function _renderMatchCard(tpl) {
    ...
    const profiles = (tpl.profile_names || []).map(p =>
      `<span class="px-1.5 py-0.5 rounded bg-purple-900/40 border border-purple-500/20 text-xs text-purple-300 font-mono">${escapeHtml(p)}</span>`
    ).join(' ');

    card.innerHTML = `
      <div class="text-sm font-semibold text-white font-mono">${escapeHtml(tpl.name)}</div>
      ${tpl.vendor ? `<div class="text-xs text-gray-400">${escapeHtml(tpl.vendor)}</div>` : ''}
      ${tpl.description ? `<div class="text-xs text-gray-300 mt-0.5 leading-relaxed">${escapeHtml(tpl.description)}</div>` : ''}
      ${rules    ? `<div class="flex flex-wrap gap-1 mt-1.5">${rules}</div>`    : ''}
      ${profiles ? `<div class="flex flex-wrap gap-1 mt-1">${profiles}</div>` : ''}
    `;
  }
