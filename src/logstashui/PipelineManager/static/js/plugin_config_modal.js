...
window.PluginConfigModal = (function () {
    ...
    `;
        ...
        if (pluginInfo.options && Object.keys(pluginInfo.options).length > 0) {
          ...
          `;
                    ...
                    const isPlaintext = value && !String(value).startsWith('${');
                    inputField = `
            <div id="${warningId}" class="${isPlaintext ? '' : 'hidden'} flex items-start gap-2 mb-2 p-3 bg-amber-900/30 border border-amber-600/50 rounded text-amber-300 text-xs">
    ...
    function checkPasswordPlaintext(warningId, value) {
...
window.togglePasswordVisibility = function (fieldId, button) {
    ...
    if (input.type === 'password') {
        input.type = 'text';
        // Change icon to eye-slash (password is now visible)
        button.innerHTML = `
            <svg class="w-5 h-5 eye-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        input.type = 'password';
        // Change icon back to eye (password is now hidden)
        button.innerHTML = `
            <svg class="w-5 h-5 eye-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
