...
"""Django settings for LogstashUI.

Values come from environment variables (see ``LogstashUI.config`` and
``LogstashUI.database``). There is no YAML config and no DATABASE_URL.
"""
...
def _get_version():
    ...
    Returns:
        Dist version, ``project.version`` from pyproject.toml, or
        ``0.0.0+unknown`` if neither is available.
    """
    try:
